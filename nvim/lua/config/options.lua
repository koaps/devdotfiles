...
-- Grepping
vim.opt.grepprg = "rg --vimgrep" -- User ripgrep if available
...
vim.opt.autoread = false -- Don't auto-save on some events
vim.opt.diffopt:append("vertical") -- Vertical diff splits
vim.opt.diffopt:append("algorithm:patience") -- Better diff algorithm
vim.opt.diffopt:append("linematch:60") -- Better diffhighlighting (smart line matching)
...
local undodir_path = vim.fn.expand(undodir) -- Undo directory path (expanded)
...
vim.opt.path:append("**") -- Search into subdirectories with `gf`
